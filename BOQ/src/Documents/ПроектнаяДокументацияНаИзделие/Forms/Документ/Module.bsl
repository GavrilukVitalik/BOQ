...
Процедура ВопросПрерватьОбработку ( Ответ, Парамы ) Экспорт
	...
	Если ( Ответ = КодВозвратаДиалога.Да ) Тогда
		...
		п.Вставить ( "УчетнаяЗапись", Объект.УчетнаяЗаписьBIM );
		п.Вставить ( "ИмяКонтейнера", Объект.ИмяКонтейнера );
		результат = прерватьОбработкуСервер ( п );
		установитьДоступность ();
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция прерватьОбработкуСервер ( Параметры )
	
	Возврат МоделиBIM.ПрерватьОбработкуМодели ( Параметры );	
...
Процедура ВопросУдалитьРезультатыОбработки ( Ответ, Парамы ) Экспорт
	...
	Если ( Ответ = КодВозвратаДиалога.Да ) Тогда
		очиститьРезультатыОбработки ();  
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура очиститьРезультатыОбработки ()
	
	Объект.ИдентификаторМодели = "";
	Объект.МодельID = 0;
	// нужно ли очищать данные в Postgres???????
...
Процедура ОткрытьПроект ( Команда )
	...
	ПоказатьПредупреждение ( , "Команда находится в разработке!" );	
	
КонецПроцедуры

&НаКлиенте
Процедура ЛистыИВидыПриАктивизацииСтроки ( Элемент )
	
	Если Элементы.ЛистыИВиды.ТекущиеДанные <> Неопределено Тогда
		установитьОтборНаИзделия  ();
	КонецЕсли;
...
&НаКлиенте
Процедура установитьОтборНаИзделия ()
