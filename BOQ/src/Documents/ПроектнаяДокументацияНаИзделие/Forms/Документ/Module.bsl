...
Процедура ОтправитьRVTНаFTP ( Команда )
	...
	Если ( Объект.ФайлRVT = "" ) Тогда
		Сообщения.СообщениеВывести ( "НеЗагруженФайлRVT" );
	Иначе
		Сообщения.ВопросПоказать ( "ВыгрузитьФайлRVTНаFTP", , "ВопросВыгрузитьФайлRVTНаFTPОбработку", ЭтотОбъект );		
	КонецЕсли;	
...
Процедура ВопросВыгрузитьФайлRVTНаFTPОбработку ( Ответ, Парамы ) Экспорт
	...
	Если ( Ответ = КодВозвратаДиалога.Да ) Тогда
		п = Новый Структура ();
		п.Вставить ( "Ссылка", Объект.Ссылка );
		п.Вставить ( "УчетнаяЗаписьBIM", Объект.УчетнаяЗаписьBIM );
		п.Вставить ( "ФайлRVT", Объект.ФайлRVT );
		выгрузитьФайлRVTНаFTPСервер ( п );	
	КонецЕсли;
			
КонецПроцедуры

&НаСервереБезКонтекста
Процедура выгрузитьФайлRVTНаFTPСервер ( Параметры )
	
	соединение = получитьFTPСоединение ( Параметры.УчетнаяЗаписьBIM );
	полноеИмяФайла = РегистрыСведений.Файлы.ПолучитьПапкуХранения ( Параметры.Ссылка ) + Параметры.ФайлRVT;
	соединение.УстановитьТекущийКаталог ( "/" );
	соединение.Записать ( "", полноеИмяФайла );		
			
КонецПроцедуры

&НаСервереБезКонтекста
Функция получитьFTPСоединение ( УчетнаяЗаписьBIM )

	п = получитьПараметрыПодключенияFTP ( УчетнаяЗаписьBIM );
	Возврат Новый FTPСоединение ( п.FTP_Путь, п.FTP_Порт, п.FTP_Логин, п.FTP_Пароль);
	
КонецФункции

&НаСервереБезКонтекста
Функция получитьПараметрыПодключенияFTP ( УчетнаяЗапись )
	
	м = Новый Массив ();
	м.Добавить ( "FTP_Логин" );
	м.Добавить ( "FTP_Пароль" );
	м.Добавить ( "FTP_Порт" );
	м.Добавить ( "FTP_Путь" ); 
